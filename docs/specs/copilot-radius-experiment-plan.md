...
Upstream declares memory limits on all 28 services and CPU limits on none. The derived copy adds a CPU limit to every service. The limits are generous rather than tight: fitted from the kernel's own CPU accounting sampled at a fine interval, because `docker stats` averages away short bursts, and accepted only if no service is throttled in healthy cycles or during startup. A limit that binds in healthy operation adds throttling noise to every measurement and buys nothing. The sum of the limits may exceed the host's cores, since a limit is not a reservation. Two reasons decide it. CPU-limit incidents change a value, and if only the faulted service carried a limit, its presence would be the clue. And 28 services competing freely for the host's cores add variance that tolerances would otherwise absorb. A limit that binds could manufacture a fault nobody injected, so incident validation records throttling on every service during each incident, and an incident whose target is throttled when its declared cause is something else is retuned or dropped.
